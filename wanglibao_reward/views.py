...
from wanglibao_account.cooperation import get_first_purchase_record
...
class RuiKeDistributer(RewardDistributer):
    def __init__(self, request, kwargs):
...
class CheFangDaiDistributer(RewardDistributer):
    def __init__(self, request, kwargs):
        super(CheFangDaiDistributer, self).__init__(request, kwargs)
        self.amount = kwargs['amount']
        self.order_id = kwargs['order_id']
        self.user = kwargs['user']
        self.token = 'cfd'
...
class CheFangDaiAPIView(APIView):
    ...
    def __init__(self):
        super(CheFangDaiAPIView, self).__init__()
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if result_no == 3:
                    ...
                    #满足条件发加息券
                    redpack_backends.give_activity_redpack(request.user, redpack_event, 'pc', send_message=False)
...
class XunleiTreasureAPIView(APIView):
    ...
    def get(self, request):
        ...
        if _type == "orders":
            records = WanglibaoActivityReward.objects.only('user__id', 'p2p_amount', 'user__wanglibaouserprofile__phone') \
                .select_related('user__wanglibaouserprofile') \
                .filter(activity=self.activity_name, p2p_amount__gt=0, left_times=0).order_by('-id')[:50]
...
class BaoGeAPIView(APIView):
    ...
    def post(self, request):
        ...
        Introducedby = IntroducedBy.objects.filter(user_id=request.user.id).first()
        if not (Introducedby and Introducedby.channel and Introducedby.channel.name == 'bg'):
            json_to_response = {
                'ret_code': 1001,
                'message': u'您不符合领奖规则'
            }
...
class JiaKeAPIView(APIView):
    ...
    def post(self, request):
        ...
        Introducedby = IntroducedBy.objects.filter(user_id=request.user.id).first()
        if not (Introducedby and Introducedby.channel and Introducedby.channel.name == 'jkdx'):
            json_to_response = {
                'ret_code': 1001,
                'message': u'您不符合领奖规则'
            }
