...
def deposit_succeed(amount):
    return u'充值操作已成功，充值金额%s元。' % str(amount)


@suffix
def withdraw_failed(error_message):
    if len(error_message) < 1:
        return u'提现失败，请重新尝试。如有疑问请致电网利宝客服电话：4008-588-066'
    return u'提现失败，原因如下：%s。如有疑问请致电网利宝客服电话：4008-588-066' % error_message
...
def withdraw_submitted(amount, issue_time):
    ...
    return u'提现申请已受理，1-3个工作日内将处理完毕，请耐心等待。'


@suffix
def withdraw_confirmed(amount):
    return u'提现成功，提现金额：%s元，请注意查收！' % amount
...
def product_failed(product):
    return u'%s[%s]在%s之前未满标，投标失败。投标账款已退回到您的网利宝平台账户中。' % (product.short_name,
                                                      product.serial_number,
...
def validate_code(code):
    return u'您的验证码为:%s，请尽快完成操作。' % code
