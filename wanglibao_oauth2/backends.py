...
from .forms import ClientAuthForm, PublicPasswordGrantForm, BajinsheClientAuthForm
...
class AccessTokenBackend(object):
    ...
    def authenticate(self, access_token=None, client=None):
        try:
            return AccessToken.objects.get(token=access_token, expires__gt=now(), client=client)
