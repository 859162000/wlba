...
class PurchaseP2PMobile(APIView):
    ...
    def _check_invalid_new_user_product(self, p2p, user):
        """
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if self._check_invalid_new_user_product(p2p, request.user):
                return Response(
                    {
                        'message': u'只有未进行投资的用户才可以购买单笔最高限购{per_total_amount}'.format(per_total_amount=p2p.limit_amount_per_user),
