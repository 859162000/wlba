...
def get_user_p2p_total_asset(user):
    ...
    other_amount = user.margin.other_amount  # P2P基金
...
def generate_bisouyi_product_data(product, action):
    ...
    if action == 'info':
        ...
        detail_url = settings.WLB_URL + product.get_detail_url_for_get('bisouyi')
        product_data = {
            ...
            'rate': float(product.expected_earning_rate) + float(product.activity_amount),
            'period': period,
            'unit': unit,
