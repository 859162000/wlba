...
from wanglibao_p2p.models import P2PProduct, P2PProductForList
...
# ~1.1s
#    p2p_all = P2PProductForList.objects \
# ~1.4s
#    p2p_all = P2PProduct.objects.defer('extra_data') \
# ~2.0s
#    p2p_all = P2PProduct.objects \

    p2p_all = P2PProduct.objects.defer('extra_data') \
        .select_related('warrant_company', 'types', 'activity__rule') \
...
def get_name_contains_p2p_list(name_contains):
    cache_backend = redis_backend()
    p2p_all = P2PProduct.objects.defer('extra_data') \
        .select_related('warrant_company', 'types', 'activity__rule') \
        .filter(name__contains=name_contains).filter(hide=False).filter(Q(status_int__gte=6)).filter(Q(publish_time__lte=timezone.now())) \
        .order_by('-status_int', '-publish_time', '-soldout_time', '-priority')
