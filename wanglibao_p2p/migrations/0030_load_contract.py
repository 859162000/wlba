...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        platform = u'网利宝平台合同'
        template = orm.ContractTemplate.objects.filter(name=platform).first()
        if template is None:
            template = orm.ContractTemplate()
            template.name = platform
            call(["pyjade", "-o", "/tmp/platform.html", "wanglibao_p2p/templates/contract_template.jade"])
            with open('/tmp/platform.html') as f:
                template.content = f.read()
            template.save()

        baoli = orm.ContractTemplate.objects.filter(name=u'保理合同').first()
        if baoli is None:
            baoli_contract = orm.ContractTemplate()
            baoli_contract.name = u'保理合同'
            call(["pyjade", "-o", "/tmp/baoli.html", "wanglibao_p2p/templates/baoli_template.jade"])
            with open('/tmp/baoli.html') as f:
                baoli_contract.content = f.read()
            baoli_contract.save()
