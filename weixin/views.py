...
class GetAuthUserInfo(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            w_user.nickname = user_info.get('nickname', None)
            w_user.sex = user_info.get('sex', None)
            w_user.city = user_info.get('city', None)
            w_user.country = user_info.get('country', None)
            w_user.headimgurl = user_info.get('headimgurl', None)
            w_user.unionid =  user_info.get('unionid', None)
            w_user.province = user_info.get('province', None)
            w_user.save()
...
class GetUserInfo(APIView):
    ...
    def get(self, request):
        ...
        if not w_user.nickname:
            ...
            w_user.unionid = user_info.get('unionid', None)
            w_user.province = user_info.get('province', None)
            w_user.subscribe = user_info.get('subscribe', None)
            w_user.subscribe_time = user_info.get('subscribe_time', None)
            w_user.save()
        return Response(user_info)
