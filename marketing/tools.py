...
logger_marketing = logging.getLogger('marketing')
...
def calc_broker_commission(product_id):
    ...
    """
    if not product_id:
        logger_marketing.info('=calc_broker_commission= product_id=[%s] parameter not gived' % (product_id))
        return

    product = P2PProduct.objects.filter(id=product_id).first()
    if not product:
        logger_marketing.info('=calc_broker_commission= product_id=[%s] record not fouond' % (product_id))
        return

    # 判断P2P产品是否为满标审核后续状态: u'满标已审核', u'还款中', u'已完成'
    _status = product.status
    if _status not in [u'满标已审核', u'还款中', u'已完成']:
        logger_marketing.info('=calc_broker_commission= product_id=[%s] [%s] not be commission' % (product_id, _status))
        return

    _method = product.pay_method
    _period = product.period
    if _method.startswith(u"日计息") and _period <= 61 or _period <=2:
        logger_marketing.info('=calc_broker_commission= product_id=[%s] [%s:%s] not be commission' % (product_id, _method, _period))
        return
    ...
    for equity in product.equities.all():
        ...
        if first_msg:
           logger_marketing.info('=calc_broker_commission= %s' % (first_msg))
        if sec_msg:
           logger_marketing.info('=calc_broker_commission= %s' % (sec_msg))
