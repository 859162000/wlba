...
def get_cache_partners():

    if r.exists('partners'):
        partners = pickle.loads(r.get('partners'))
    else:
        ...
        r.set('partners', pickle.dumps(partners))

    return partners


def get_cache_p2p_detail(product_id):

    if r.exists('p2p_detail_{0}'.format(product_id)):
        p2p_results = pickle.loads(r.get('p2p_detail_{0}'.format(product_id)))
        return p2p_results
    else:
        try:
            ...
            if p2p.status != u'正在招标':
                r.set('p2p_detail_{0}'.format(product_id), pickle.dumps(p2p_results))
