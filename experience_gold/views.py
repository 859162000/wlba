...
class ExperienceGoldView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated():
            experience_record = ExperienceEventRecord.objects.filter(user=user, event__invalid=False)\
            ...
            if experience_amortization:
                paid_interest = reduce(lambda x, y: x + y,
                                       [e.interest for e in experience_amortization if e.settled is True])
                unpaid_interest = reduce(lambda x, y: x + y,
                                         [e.interest for e in experience_amortization if e.settled is False])

        total_experience_amount = experience_amount + paid_interest + unpaid_interest
        return {
