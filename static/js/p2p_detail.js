...
(function() {
  ...
  require(['jquery', 'underscore', 'lib/backend', 'lib/calculator', 'lib/countdown', 'tools', 'lib/modal', "jquery.validate", 'ddslick'], function($, _, backend, calculator, countdown, tool, modal) {
    ...
    $.validator.addMethod('threshold', function(value, element) {
      var l, len1, obj, selectedData;
      for (l = 0, len1 = ddData.length; l < len1; l++) {
        obj = ddData[l];
    ...
    if ($('.red-pack').size() > 0) {
      return $(document).ready(function() {
        $.post('/api/redpacket/', {
          ...
          var available_time, availables, data2, datetime, desc, l, len1, obj;
          data2 = data;
          ...
          for (l = 0, len1 = availables.length; l < len1; l++) {
            obj = availables[l];
          ...
          $('.red-pack').ddslick({
            ...
            onSelected: function(data) {
              var j, lable, pay_amount, pay_now, results, val_len;
          ...
          $('#id_amount').keyup(function(e) {
            var amount, amount2, amount3, g, k, lable, len2, m, max_pay, mes, obj_val, pay_amount, pay_now, results, selectedData, val_len2;
            max_pay = $('#id_amount').attr('data-max');
            amount2 = $('#id_amount').val();
            if (obj.value) {
              if ($('#id_amount').val() <= max_pay) {
                for (m = 0, len2 = ddData.length; m < len2; m++) {
                  obj = ddData[m];
                  if (obj.value === $('.dd-selected-value').val() * 1) {
                    selectedData = obj;
                    break;
