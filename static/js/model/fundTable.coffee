        ...
        ,
          ...
          text: (item)->
            if item.availablefund
              '<a class="button-small button-pink" href="/shumi/oauth/check_oauth_status/?fund_code=' + item.product_code + '&action=purchase" target="_blank"> 购买 </a>'
            else
              '<a class="button-small button-gray" href="javascript:void(0)"> 购买 </a>'
        ,
          name: '收藏'
          colspan: 1
          text: (item)->
            if item.is_favorited == 1
              '<a class="button-small button-white button-no-border" onclick="addToFavorite(event,' + "'funds');" +
                '" href="#" data-is-favorited=' + item.is_favorited + ' data-id="' + item.id + '">取消</a>'
            else
              '<a class="button-small button-white" onclick="addToFavorite(event,' + "'funds');" +
