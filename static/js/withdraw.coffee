      ...
      if $('.bindingCard').text() == ''
        ...
        if addFormValidateor.form()
          $('#withdraw-form').ajaxSubmit((data) ->
            if data.ret_code == 0
              tool.modalAlert({title: '温馨提示', msg: '<p style="font-size:14px;color:#333">提现申请已处理</p><p style="margin-top:5px;font-size:14px;color:#333">预计1-3个工作日到帐</p>', callback_ok: _showModal, height: '220px'})
            else
