...
class CoopDataDispatchApi(APIView):
    ...
    def check_sign(self, channel, _time, key, sign):
        local_sign = hashlib.md5(channel + key + _time).hexdigest()
        if local_sign == sign:
            return True
        else:
            return False

    def save_to_channel(self, req_data):
        response_data = {}
    ...
    def post(self, request):
        req_data = request.POST
        channel = req_data.get('channel')
        _time = req_data.get('time')
        sign = req_data.get('sign')
        act = req_data.get('act')

        if channel and _time and sign and act:
            # 判断时间戳有效性
            current_time = timezone.localtime()
            if _time.isdigit() and int(current_time) - int(_time) <= 120:
                # 判断数据来源有效性
                if channel in ('base', 'yuelibao'):
                    # 判断数据签名有效性
                    key = settings.BASE_SYNC_KEY if channel == 'base' else settings.YLB_SYNC_KEY
                    is_right_sign = self.check_sign(channel, _time, key, sign)
                    if is_right_sign:
                        # 判断动作有效性，并调度相关处理器
                        if act == 'new_channel':
                            response_data = self.save_to_channel(req_data)
                        elif act == 'register':
                            response_data = ''
                        elif act == 'purchase':
                            response_data = ''
