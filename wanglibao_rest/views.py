        ...
        if serializer.is_valid():
            ...
            if invite_code:
                try:
                    ...
                    return Response({"ret_code":30011, "message":"邀请码错误"}, status=status.HTTP_200_OK)
            ...
            return Response({"ret_code":0, "message":"注册成功"}, status=status.HTTP_200_OK)
            #return Response([{"message":u'注册成功'}], status=status.HTTP_200_OK)
        if len(serializer.errors):
            return Response({"ret_code":30012, "message":serializer.errors[0].items()[0][1]}, status=status.HTTP_200_OK)
        return Response({"ret_code":30013, "message":"未知错误"}, status=status.HTTP_200_OK)
